...
<div class="p-6 md:p-8 pb-28 flex gap-6 h-full w-full overflow-hidden relative">
    <!-- Floating Sidebar (Slides completely off-screen left) -->
    <aside
        class="p-4 rounded-3xl bg-elevation-1 flex shrink-0 flex-col gap-4 shadow-md w-64 md:w-72 squircle-smooth select-none t:(transform-200-quad-out opacity-200-quad-out) {isCollapsed
            ? '-translate-x-[calc(100%+3rem)] opacity-0 pointer-events-none absolute left-0 top-6 bottom-28'
    ...
    <!-- Main Component Stage (Expands cleanly) -->
    <main
