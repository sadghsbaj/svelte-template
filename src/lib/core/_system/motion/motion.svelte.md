...
- **FOAM Prevention (index.html Inline Script):** Cooperates with the pre-existing inline script in `index.html`. That script runs immediately inside `<head>` before page paint to add the `data-reduce-motion` attribute if a reduced motion preference is saved. During initialization, the client-side `MotionManager` immediately reads `data-reduce-motion` from the DOM to inherit this resolved state, ensuring a seamless hydration transition without visual layout jumps or unintended startup animations. Refer to `index.html` to inspect the inline script implementation.
- **Global CSS Integration:** Adding `data-reduce-motion` to `<html>` should hook into global stylesheets to immediately disable transitions and animations (e.g. `* { transition: none !important; animation: none !important; }`).
